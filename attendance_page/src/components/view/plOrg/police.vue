...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getPoliceListNotBindOrg(params){
        var data = {
        ...
        var _self = this;
        this.$http.get('/business/plorg/getpolicenotbindorg', {params: data})
          .then(function (res) {
      ...
      onSearch(){
        ...
        this.getPoliceListNotBindOrg(params);
      },
      /**
       * 保存用户角色关联信息
      ...
      submitForm(){
        ...
        if(this.sels.length>0){
          this.$http.post('/business/plorg/saveorgpolice', params)
            .then(function (res) {
              if (res.data && res.data.success) {
                _self.getPoliceListNotBindOrg();

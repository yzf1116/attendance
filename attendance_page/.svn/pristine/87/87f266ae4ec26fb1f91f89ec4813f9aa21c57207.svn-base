<template>
  <div>
    ...
    <el-dialog title="审批人员选择" :visible.sync="dialogVisible" class="groupclass" size="large"
               :close-on-click-modal="false" style="width:100%;max-height: 100%">
      <div style="display: flex">
        <div style="display:flex; width:55%;">
          <div style="display: flex;flex-wrap:wrap; width:80%">
            <draggable  :list="policeList" :move="getdata" @update="datadragEnd" :options="{animation: 300,handle:'.dargDiv'}">
              <transition-group style="display: flex;flex-wrap:wrap;overflow: hidden" name="list-complete" >
                <div style="display: flex;flex-wrap:wrap;overflow: hidden" v-for="(item, index) in policeList" :key="item.name">
                  <div class="approve dargDiv">
...
//        ],
    ...
    components: {
      ...
      ElForm,
      ElButton,
      detailForm, paging, Police,draggable
    },
    updated() {
    ...
    computed: {
      sendPersonData: function () {
       ...
       let data =  sendPerson.concat(vm.sendPersonList);
        ...
        console.log('vm.sendPersonList',vm.sendPersonList);
        console.log('data',data);
        for (let item of data) {    //过滤对象数组中重复的值

            let isAdd = true;
            for (let it of sendArr) {
    ...
    methods: {
      ...
      closeDialog(value){
        ...
        if (value === 0) {
          ...
          vm.rightCheckList = [];
          vm.dialogVisible = false;
          for (let i = 0; i < vm.policeList.length; i++) { //点击确定提交保存审批人
                if(vm.policeList[i].id !== undefined && vm.policeList.approverid !== undefined){ //这是不选择直接保存主管的判断
                  vm.policeList[i].appr_level = i + 1;
                  vm.policeList[i].temp_id = vm.ruleForm.id;
                  vm.policeList[i].templateid = vm.policeList[i].temp_id;
                }else if(vm.policeList[i].approverid === undefined){  //这是选择指定主管的判断
                  vm.policeList[i].approverid = vm.policeList[i].id;
                  vm.policeList[i].appr_level = i + 1;
                  vm.policeList[i].temp_id = vm.ruleForm.id;
                  vm.policeList[i].templateid = vm.policeList[i].temp_id;
                } else if(vm.policeList[i].id === undefined){ // 这是选择默认主管的判断
                  vm.policeList[i].appr_level = i + 1;
                  vm.policeList[i].temp_id = vm.ruleForm.id;
                  vm.policeList[i].templateid = vm.policeList[i].temp_id;
                }
...
//                    parentname: data[i].org_name,
      ...
      treeChange(data, isCheck, childerCheck){
        console.log(data,isCheck,childerCheck);
        let vm = this;
        if(isCheck){
...
//      },
       ...
       *
       * 2017年08月01日
       *          选择审批人再测一下，还有选择抄送人有重复的问题，刷新后才没有。
       *   任务：自定义表达需要做审批编码验证
       */
      //获取模版列表数据
      ...
      delSendPerson(index){ //删除抄送人
        let vm = this;
         vm.$store.getters.getApprove.splice(index,1);
        vm.sendPersonData.splice(index, 1);
...
//        console.log('警员',vm.policeData.length);
      ...
      handleSetSend(index, row){  //设置抄送人按钮
        ...
        vm.ruleForm = row;
        vm.dialogShow = true;
        console.log('打开设置后的抄送人',vm.sendPerson);
        console.log('打开设置后的',vm.sendPersonData);

        console.log('打开设置后的2',vm.sendPersonData);
        let sendPersons = [];
...
//        }
      ...
      undialogClose(ms){
        ...
        if (ms.dialog === 0) {
          ...
          let sendPerson = this.$store.getters.getApprove;
          console.log('点击取消',sendPerson);
          sendPerson = [];
          vm.$store.dispatch('setApprove', sendPerson);
          vm.getApprove();
          vm.getSendPerson();
          vm.idChangeName();
          vm._policeListData();

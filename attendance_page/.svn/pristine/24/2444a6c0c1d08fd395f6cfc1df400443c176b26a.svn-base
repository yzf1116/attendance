<template>
  <div>
    <el-form :inline="true" :model="formInline" class="demo-form-inline" ref="formInline"
             @keyup.enter.native="onSearch('formInline')">
      <el-form-item label="考勤组名称" prop="atd_name">
        <el-input v-model="formInline.atd_name" size="mini" placeholder="考勤组名称"></el-input>
      </el-form-item>
      <el-form-item>
        <el-button size="mini" type="primary" icon="search" @click="onSearch('formInline')">查询</el-button>
        <el-button size="mini" type="info" @click="resetForm('formInline')">
          <icon name="refresh" class="custom-icon"></icon>
          重置


        </el-button>
        <!--<el-button size="mini" type="danger" icon="delete" @click="batchRemove">批量删除</el-button>-->
      </el-form-item>
      <el-form-item style="margin-left: 30px;">
        <el-button size="mini" type="success" icon="plus" @click="handleAdd">新增</el-button>
      </el-form-item>
    </el-form>

    <el-table :data="groupTableData" border v-loading="loading" :height="this.$store.state.gTableHeight"
              style="width: 100%">
      <el-table-column prop="atd_name"  label="考勤组名称" sortable>
      </el-table-column>
      <el-table-column label="人数" :formatter="countFormatter">
      </el-table-column>
      <el-table-column :formatter="typeFormatter" label="类型">
      </el-table-column>
      <el-table-column width="400" label="考勤时间">
        <template scope="scope">
          <div v-show="scope.row.type==='1'">
            <p>{{timeFormatter(scope.row, 'on')}}</p>
            <p>{{timeFormatter(scope.row, 'off')}}</p>
          </div>
          <div v-show="scope.row.type==='2'">
              <el-tag type="gray" v-for="(item,index) in scope.row.atd_schedule_groups" :close-transition="true" style="margin:0 3px;"  :key="item.id">{{item.atd_schedule.name}}</el-tag>
          </div>
          <div v-show="scope.row.type==='3'"></div>
        </template>
      </el-table-column>
      <el-table-column label="操作" width="250">
        <template scope="scope">
          <el-button size="mini" @click="handleEdit(scope.$index, scope.row)">编辑
          </el-button>
          <el-button size="mini" type="danger" @click="handleDelete(scope.$index, scope.row)">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    <paging @emitsizechange="handleSizeChange" @emitcurrentchange="handleCurrentChange"
            :currentPage="tabPage.currentPage" :pageSizes="tabPage.pageSizes" :pageSize="tabPage.pageSize"
            :total="tabPage.totalNum">
    </paging>
  </div>
</template>

<script>
  import moment from 'moment'
  import paging from '../../common/Paging.vue'
  import ElFormItem from "../../../../node_modules/element-ui/packages/form/src/form-item";
  export default {
    data() {
      return {
        groupTableData: [],
        sels: [],//表格选中列
        tabPage: {
          currentPage: 1,
          pageSize: 10,
          pageSizes: [10, 20, 30, 50]
        },//分页信息
        loading: true,
        formInline: {
          atd_name: ''
        }
      }
    },
    methods: {
      /**
       * 表单重置
       * @params {Object} formName 表单名称
       */
      resetForm(formName) {
        this.$refs[formName].resetFields();
        if (formName == 'formInline') {
          this.getGroupInfo();
        }
      },

      /**
       *查询考勤组
       * @param formName 表单
       */
      onSearch(formName) {
        var params = this.formInline;
        params.page = 1;
        params.pageSize = this.tabPage.pageSize;
        this.tabPage.currentPage = 1;//每次查询默认第一页
        var _self = this;
        _self.getGroupInfo(params);

      },

      /**
       * 编辑按钮点击事件
       * @params {Number} index  行号
       * @params {Object} row 行对象
       */
      handleEdit(index, row) {
        this.$router.push({path: 'newAtdGroups', query: {id: row.id}})

      },
      /**
       * 删除按钮点击事件
       * @params {Number} index   行号
       * @params {Object} row     行对象
       */
      handleDelete(index, row) {
        var _self = this;
        console.log(row)
        _self.deleteGroupnfo(row);
      },
      /**
       * 删除考勤组信息
       * @params {Object} row|rows     行对象
       */
      deleteGroupnfo(data) {
        var _self = this;
        this.$confirm('此操作将永久删除选择考勤组, 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(function () {
          _self.$http.post('/business/attendancegroup/deletegroupinfo', data).then(function (res) {
            if (res.data && res.data.success) {
              _self.$message({
                message: '删除成功',
                type: 'success'
              });
              _self.getGroupInfo();
            } else {
              _self.$message({
                message: res.data.msg,
                type: 'warning'
              });
            }
          }).catch(function (err) {
            console.log(err);
          })
        }).catch(function (err) {
          console.log(err);
        });
      },
      /**
       *格式化时间
       * @param  {Object}  row
       */
      dateFormatter(row) {
        return moment(row.work_time).format('HH:mm') + '-' + moment(row.off_work_time).format('HH:mm');
      },

      /**
       * 考勤时间
       * @param {Object}  row
       */
      timeFormatter(row, obj) {
        var total = 0;
        var text = '';
        var offtext = '';
        const scheduleInfo = row.atd_schedule_plans;
        for (const item of scheduleInfo) {
          switch (item.week_calendar) {
            case 'week1':
              item.week_calendar = '周一';
              break;
            case 'week2':
              item.week_calendar = '周二';
              break;
            case 'week3':
              item.week_calendar = '周三';
              break;
            case 'week4':
              item.week_calendar = '周四';
              break;
            case 'week5':
              item.week_calendar = '周五';
              break;
            case 'week6':
              item.week_calendar = '周六';
              break;
            case 'week7':
              item.week_calendar = '周日';
              break;
          }
          if (item.is_rest == 1) {
            text = text + item.week_calendar + '、';
          } else {
            offtext = offtext + item.week_calendar + '、'
          }
        }
        if (obj == 'on') {
          let value = '';
          if (text.length > 0) {
            return `${text.substring(0, text.length - 1)}休息`
          } else {
            return ''
          }
        }
        if (obj == 'off') {
          let value = '';
          if (offtext.length > 0) {
            return `${offtext.substring(0, offtext.length - 1)}工作`
          } else {

          }
        }

      },

      /**
       *考勤类型
       * @param row [Object]当前前行
       */
      typeFormatter(row, column) {
        var TYPE = '';
        switch (row.type) {
          case '1':
            TYPE = '固定班制';
            break;
          case '2':
            TYPE = '排班制';
            break;
          case '3':
            TYPE = '自由工时';
            break;
        }
        return TYPE;
      },

      /**
       * 考勤人数
       * @param row {Object} 当前行
       */
      countFormatter(row, column) {
        return row.pl_people.length + '人'
      },
      /**
       * 查询所有班组信息
       * @param params {Object} 用户查询信息
       */
      getGroupInfo(params) {
        var _self = this;
        _self.loading = true;
        var data = {
          page: this.tabPage.currentPage,
          pageSize: this.tabPage.pageSize,
          atd_name: this.formInline.atd_name
        };
        if (params) {
          data = params;
        }
        this.$http.get('/business/attendancegroup/getgrouplist', {params: data})
          .then(function (res) {
            if (res.data && res.data.success) {
              _self.groupTableData = res.data.result.rows;
              _self.tabPage.totalNum = res.data.result.count;
              _self.loading = false;
            } else {
              _self.loading = false;
              _self.$message({
                message: res.data.msg,
                type: 'warning'
              });
            }
            console.log(res)
          }).catch(function (error) {
          _self.loading = false;
          console.log(error)
        });
      },

      /**
       * 点击新增按钮
       */
      handleAdd() {
        this.$router.push({path: 'newAtdGroups'})
      },
      /**
       * 批量删除操作
       */
      batchRemove() {
        var _self = this;
        var arr = _self.sels;
        if (arr.length > 0) {
          _self.getGroupInfo({params: arr});
        } else {
          _self.$message({
            message: '请选择考勤组',
            type: 'warning'
          });
        }

      },
      /**
       * 切换每页条数
       * @params {Number} val 每页条数
       */
      handleSizeChange(val) {
        this.tabPage.pageSize = val;
        this.getGroupInfo();
      },
      /**
       * 切换页码
       * @params {Number} val 页码
       */
      handleCurrentChange(val) {
        this.tabPage.currentPage = val;
        this.getGroupInfo();
      },
    },
    components: {ElFormItem, paging},
    created() {
      this.getGroupInfo();
    },
    mounted() {
      console.log('mounted')
    },
    updated() {
      console.log('updated')
    },
  }
</script>

<!-- 添加 "scoped " css作用域只作用于本文件，不作用全局-->
<style scoped>

</style>
